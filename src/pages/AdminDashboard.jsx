...
export default function AdminDashboard() {
  ...
  const sendConfirmationEmail = async (userEmail, userPassword, userRole, assignedSites) => {
    try {
  ...
  const getIsAdmin = (user) => getUserRole(user) === 'admin'
  const getInitial = (user) => {
    const name = user?.email || 'U'
    const first = String(name).trim().charAt(0)
    return first ? first.toUpperCase() : 'U'
  }
  const getUserSites = (userId) => userSites.filter((us) => us.user_id === userId)
  ...
  const handleRemoveSiteAssignment = async (userId, siteId) => {
    try {
      const { data, error } = await supabase.rpc('remove_site_assignment', { user_id: userId, site_id: siteId })
      if (error) throw error
      if (data.success) {
  ...
  const handleEditSiteToggle = (siteId) => {
    setEditSelectedSites((prev) => (prev.includes(siteId) ? prev.filter((id) => id !== siteId) : [...prev, siteId]))
  }
    ...
    <div className="min-h-screen bg-background">
      <header className="bg-card border-b border-border p-6">
        <div className="w-full flex justify-between items-center">
          ...
          <div className="flex items-center gap-3">
            <button onClick={handleLogout} className="flex items-center gap-2 px-3 py-2 rounded-lg text-sm text-red-600 border border-red-300 hover:bg-red-50 dark:border-red-700 dark:hover:bg-red-900/30 transition-colors">
      ...
      <main className="flex-1 overflow-auto">
        <div className="max-w-[76rem] mx-auto p-6 space-y-6">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            {!showForm && (
              <button onClick={() => setShowForm(true)} className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-black text-white font-medium shadow-sm hover:bg-black/90 active:scale-[0.99] transition-all">
                <UserPlus className="w-5 h-5" />
                Add New User
              </button>
            )}
            {!showSiteForm && (
              <button onClick={() => setShowSiteForm(true)} className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl border border-border hover:bg-muted active:scale-[0.99] transition-all">
            ...
            <div className="bg-card/70 backdrop-blur-sm rounded-2xl border border-border p-6 shadow-sm">
              ...
              <form onSubmit={handleCreateUser} className="space-y-6">
                <div>
                  ...
                  <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full px-3 py-2.5 rounded-xl border border-border focus:outline-none focus:ring-4 focus:ring-black/10 focus:border-black bg-background text-foreground transition-colors" placeholder="user@example.com" required disabled={creating} />
                </div>
                <div>
                  <label className="block text-sm font-medium text-foreground mb-2">Password</label>
                  <div className="flex gap-2">
                    <div className="relative flex-1">
                      <input type={showPassword ? 'text' : 'password'} value={password} onChange={(e) => setPassword(e.target.value)} className="w-full px-3 py-2.5 pr-10 rounded-xl border border-border focus:outline-none focus:ring-4 focus:ring-black/10 focus:border-black bg-background text-foreground transition-colors" placeholder="Enter password" required minLength={6} disabled={creating} />
                      <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 p-1 rounded-lg hover:bg-muted transition">
                        {showPassword ? <EyeOff size={18} /> : <Eye size={18} />}
                      </button>
                    </div>
                    <button type="button" onClick={generatePassword} className="px-4 py-2.5 rounded-xl border border-border hover:bg-muted active:scale-[0.99] transition-all whitespace-nowrap" disabled={creating}>Generate</button>
                    <button type="button" onClick={copyPassword} className="p-2.5 rounded-xl border border-border hover:bg-muted active:scale-[0.99] transition-all" disabled={!password || creating}>{copied ? <Check size={18} className="text-green-600" /> : <Copy size={18} />}</button>
                  </div>
                  <p className="text-xs text-muted-foreground mt-1">Minimum 6 characters</p>
                </div>
                <div>
                  <label className="block text-sm font-medium text-foreground mb-2">Role</label>
                  <select value={userRole} onChange={(e) => setUserRole(e.target.value)} className="w-full px-3 py-2.5 rounded-xl border border-border focus:outline-none focus:ring-4 focus:ring-black/10 focus:border-black bg-background text-foreground transition-colors" disabled={creating}>
                    ...
                    {sites.length === 0 ? (
                      ...
                      sites.map((site) => (
                        <label key={site.id} className="flex items-center gap-2 cursor-pointer">
                          <input type="checkbox" checked={selectedSites.includes(site.id)} onChange={() => handleSiteToggle(site.id)} className="rounded border-border" disabled={creating} />
                ...
                <div className="flex flex-col sm:flex-row gap-3">
                  <button type="submit" disabled={creating} className="inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-black text-white font-medium shadow-sm hover:bg-black/90 disabled:opacity-60 active:scale-[0.99] transition-all">
                    {creating ? (<><Loader2 className="animate-spin" size={18} />Creating...</>) : (<><UserPlus size={18} />Create User</>)}
                  </button>
                  <button type="button" onClick={resetForm} className="px-4 py-2.5 rounded-xl border border-border hover:bg-muted active:scale-[0.99] transition-all" disabled={creating}>Cancel</button>
                </div>
              </form>
            </div>
          )}
            ...
            <div className="bg-card/70 backdrop-blur-sm rounded-2xl border border-border p-6 shadow-sm">
              ...
              <form onSubmit={handleCreateSite} className="space-y-6">
                <div>
                  ...
                  <input type="text" value={siteName} onChange={(e) => setSiteName(e.target.value)} className="w-full px-3 py-2.5 rounded-xl border border-border focus:outline-none focus:ring-4 focus:ring-black/10 focus:border-black bg-background text-foreground transition-colors" placeholder="site-name" required />
                  <p className="text-xs text-muted-foreground mt-1">Internal identifier (lowercase, no spaces)</p>
                </div>
                <div>
                  <label className="block text-sm font-medium text-foreground mb-2">Display Name</label>
                  <input type="text" value={siteDisplayName} onChange={(e) => setSiteDisplayName(e.target.value)} className="w-full px-3 py-2.5 rounded-xl border border-border focus:outline-none focus:ring-4 focus:ring-black/10 focus:border-black bg-background text-foreground transition-colors" placeholder="Site Display Name" required />
                  <p className="text-xs text-muted-foreground mt-1">Human-readable name shown in the interface</p>
                </div>
                <div className="flex flex-col sm:flex-row gap-3">
                  <button type="submit" className="inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-black text-white font-medium shadow-sm hover:bg-black/90 active:scale-[0.99] transition-all">
                    <MapPin size={18} />
                    Create Site
                  </button>
                  <button type="button" onClick={() => { setShowSiteForm(false); setSiteName(''); setSiteDisplayName(''); setError(null); setSuccess(null) }} className="px-4 py-2.5 rounded-xl border border-border hover:bg-muted active:scale-[0.99] transition-all">Cancel</button>
                </div>
              </form>
            </div>
          )}
          ...
          <div className="bg-card/70 backdrop-blur-sm rounded-2xl border border-border overflow-hidden shadow-sm">
            ...
            {loading ? (
              ...
              <div className="p-8 text-center text-muted-foreground">No users found. Create your first user above.</div>
            ) : (
                      ...
                      <tr key={user.id} className="hover:bg-muted/30 transition">
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="flex items-center gap-2">
                            <div className="w-8 h-8 rounded-full bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center text-white text-sm font-medium">{getInitial(user)}</div>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          {(() => {
                            ...
                            if (role === 'admin') return (<span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-medium bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-300"><Shield size={12} />Admin</span>)
                            if (role === 'reviewer') return (<span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300"><Users size={12} />Reviewer</span>)
                            return (<span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-muted text-foreground">User</span>)
                          })()}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                            ...
                            {getUserSites(user.id).length === 0 ? (
                              ...
                              getUserSites(user.id).map((userSite) => (
                                <span key={userSite.site_id} className="inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-300">
                                  ...
                                  <button onClick={() => handleRemoveSiteAssignment(user.id, userSite.site_id)} className="ml-1 hover:bg-orange-200 dark:hover:bg-orange-800 rounded-full p-0.5 transition" title="Remove site assignment">
                                    <X size={8} />
                                  </button>
                                </span>
                              ))
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-muted-foreground">{user?.created_at ? new Date(user.created_at).toLocaleDateString() : '-'}</td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="flex items-center gap-2">
                            <button onClick={() => handleEditUser(user)} className="p-2 rounded-lg text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/30 transition-colors" title="Edit user">
                              <Edit size={16} />
                            </button>
                            <button onClick={() => handleDeleteUser(user)} disabled={deleting} className="p-2 rounded-lg text-red-600 hover:bg-red-50 dark:hover:bg-red-900/30 transition-colors disabled:opacity-50" title="Delete user">
          ...
          <div className="bg-card/70 backdrop-blur-sm rounded-2xl border border-border overflow-hidden shadow-sm">
            ...
            {sites.length === 0 ? (
              <div className="p-8 text-center text-muted-foreground">No sites found. Create your first site above.</div>
            ) : (
              <div className="overflow-x-auto -mx-6 sm:mx-0">
                  ...
                  <tbody className="divide-y divide-border/50">
                    {sites.map((site) => (
                      <tr key={site.id} className="hover:bg-muted/30 transition">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${site.is_active ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-300' : 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-300'}`}>
                            {site.is_active ? 'Active' : 'Inactive'}
                          </span>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-muted-foreground">{site.created_at ? new Date(site.created_at).toLocaleDateString() : '-'}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
